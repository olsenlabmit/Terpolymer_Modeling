...
combos = pd.read_csv('C:/Users/ChemeGrad2020/Documents/Research/Ter-Polymer_Library_Biodeg/Library_plan.csv')
...
with open('Library_Synthesis.csv', mode='w') as file:
    ...
    for i in combos.index:
        ...
        if combos['Diol 1'][i]==diol_sorted['Carboxylic Acid or Diol'][ind] or combos['Diol 2'][i] == diol_sorted['Carboxylic Acid or Diol'][ind]:
            ...
            for j in range(len(CAs_sorted.index)):
                name+=str(CAs_sorted['Polymer Name Abbr.'][CAs_sorted.index[j]])+'_'
            ...
            for j in range(len(DLs_sorted.index)):
                name+=str(DLs_sorted['Polymer Name Abbr.'][DLs_sorted.index[j]])+'_'
            name+='P'
            #calculate synthesis set
            syn_set=(len(polymer_numbers)//11)+1
            
            #write the information into the csv file
            with open('Library_Synthesis.csv', mode='a', newline='') as file:
                file_writer=csv.writer(file, delimiter=',')
...
df=pd.read_csv('Library_Synthesis.csv',header=0)
