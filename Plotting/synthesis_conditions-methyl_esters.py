...
with open('Library_Synthesis_methyl_esters_Julia.csv', mode='w') as file:
...
for ind in diol_sorted.index:
    ...
    for i in combos.index:
        ...
        if combos['Polymer Number'][i] in polymer_numbers:
            continue
        if combos['Carboxylic Acid 1'][i] not in ['terephthalic acid','isophthalic acid'] and combos['Carboxylic Acid 2'][i] not in ['terephthalic acid', 'isophthalic acid']:
            continue
        if combos['Diol 1'][i]==diol_sorted['Carboxylic Acid or Diol'][ind] or combos['Diol 2'][i] == diol_sorted['Carboxylic Acid or Diol'][ind]:
            ...
            #now we need to replace any terephthalic or isophthalic acid row with DMT or DMI
            #print(monomers[monomers['Carboxylic Acid or Diol']=='Dimethyl Terephthalate'])
            #print(CAs_sorted.loc[CAs_sorted['Carboxylic Acid or Diol']=='isophthalic acid'].empty)
            if CAs_sorted.loc[CAs_sorted['Carboxylic Acid or Diol']=='isophthalic acid'].empty:
                print(CAs_sorted.drop(index=CAs_sorted.loc[CAs_sorted['Carboxylic Acid or Diol']=='terephthalic acid'].index))
                CAs_sorted=CAs_sorted.drop(index=CAs_sorted.loc[CAs_sorted['Carboxylic Acid or Diol']=='terephthalic acid'].index)
                CAs_sorted=CAs_sorted.append(monomers[monomers['Carboxylic Acid or Diol']=='Dimethyl Terephthalate'])
                print(CAs_sorted)
            if CAs_sorted.loc[CAs_sorted['Carboxylic Acid or Diol']=='Dimethyl Terephthalate'].empty:
                #print(CAs_sorted.drop(index=CAs_sorted.loc[CAs_sorted['Carboxylic Acid or Diol']=='isophthalic acid'].index))
                CAs_sorted=CAs_sorted.drop(index=CAs_sorted.loc[CAs_sorted['Carboxylic Acid or Diol']=='isophthalic acid'].index)
                CAs_sorted=CAs_sorted.append(monomers[monomers['Carboxylic Acid or Diol']=='Dimethyl Isophthalate'])
            print(CAs_sorted)
            #CAs_sorted.loc[(CAs_sorted['Carboxylic Acid or Diol']=='isophthalic acid')]=(monomers.loc[monomers['Carboxylic Acid or Diol']=='Dimethyl Isophthalate'])
            #print(CAs_sorted)
            ...
            print(len(DLs_sorted.index))
            #write the information into the csv file
            with open('Library_Synthesis_methyl_esters_Julia.csv', mode='a', newline='') as file:
                file_writer=csv.writer(file, delimiter=',')
...
df=pd.read_csv('Library_Synthesis_methyl_esters_Julia.csv',header=0)
